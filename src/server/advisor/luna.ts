...
Some candidates contain a family with concrete members. Treat a family as one conceptual recommendation, but when recommending it, explain the individual member upgrades to the user and return their exact IDs in memberCandidateIds. Do not hide the per-item breakdown behind only an aggregate. Family totals summarize the supplied members; each member remains independently actionable.
