...
export function buildProfileIntelligence(profile: NormalizedSkyBlockProfile): ProfileIntelligenceSnapshot {
  ...
  return {
    ...
    domains: {
      ...
      MINING: { domain: "MINING", miningLevel: compactLevel(profile.progression.skills.mining), hotmLevel: null,
        mithrilPowder: null, gemstonePowder: null, tools: miningTools.map(compact), armor: miningArmor.map(compact),
        ...
        unavailableFacts: ["Heart of the Mountain level is unavailable in the current data contract.",
          "Mithril and Gemstone powder are unavailable in the current data contract.",
