...
Phase 5.1 expands the normalized snapshot rather than adding another data source. Domain builders now read mining tree XP, powders, nodes and selections; fishing counters and trophy-fish state; and accessory powers and tuning from the cached normalized profile. Foraging tree and core state is normalized for a later routed domain.
